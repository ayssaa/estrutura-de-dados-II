...
void main() {
    ...
    // Tabela do mau caractere (Horspool: tabela de deslocamento)
    int charuim[alfabeto];
    for (int i = 0; i < alfabeto; i++) {
        charuim[i] = m;                 // default: pula m posições se char não está no padrão
    }
    for (int i = 0; i < m - 1; i++) {
        charuim[(unsigned char)padrao[i]] = m - 1 - i; // mapeia deslocamentos para as m-1 primeiras posições
    }
    ...
    while (shift <= n - m) {
        ...
        if (j < 0) {
            ...
            // Horspool: desloca usando o caractere do fim da janela atual
            unsigned char prox = (unsigned char)texto[shift + m - 1];
            shift += charuim[prox];
        }
    } 
